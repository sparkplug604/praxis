...
def builtin_manifests() -> dict[str, QueryManifest]:
    manifests = [
        ...
        QueryManifest(
            manifest_id="warehouse_segment_size_preview",
            description="Preview aggregate segment/list size and suppression hygiene from a read-only warehouse table.",
            requires=["warehouse"],
            parameters=["client_id"],
            outputs=["contacts", "accounts", "segment_size", "suppressed_count", "missing_email_count", "stale_record_count"],
            storage_level="aggregate_summary",
            pii_policy="none",
            conflict_checks=["stale_source_warning", "suppression_warning"],
            source_priority=["warehouse"],
            context_template="weekly_gtm_review",
        ),
        QueryManifest(
            manifest_id="warehouse_buyer_signal_rollup",
            description="Roll up buyer-signal counts and average signal strength from a read-only warehouse table.",
            requires=["warehouse"],
            parameters=["client_id", "start_date", "end_date"],
            outputs=["buyer_signal_count", "avg_signal_strength", "accounts", "contacts"],
            storage_level="aggregate_summary",
            pii_policy="none",
            conflict_checks=["stale_source_warning"],
            source_priority=["warehouse"],
        ...
        QueryManifest(
            manifest_id="warehouse_suppression_check",
            description="Check aggregate suppression and missing-contact-data counts before list activation.",
            requires=["warehouse"],
            parameters=["client_id"],
            outputs=["contacts", "suppressed_count", "missing_email_count"],
            storage_level="aggregate_summary",
            pii_policy="none",
            conflict_checks=["suppression_warning"],
            source_priority=["warehouse"],
            context_template="weekly_gtm_review",
        ),
        QueryManifest(
            manifest_id="warehouse_campaign_outcome_rollup",
            description="Roll up campaign outcomes from a read-only warehouse table for segment feedback loops.",
            requires=["warehouse"],
            parameters=["client_id", "start_date", "end_date"],
            outputs=["leads", "opportunities", "pipeline_amount", "closed_won_revenue", "conversion_rate"],
            storage_level="aggregate_summary",
            pii_policy="none",
            conflict_checks=["stale_source_warning"],
            source_priority=["warehouse"],
