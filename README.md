...
```mermaid
flowchart LR
    A["Documents / Research / Notes"] --> B["Praxis Core"]
    B --> C["Searchable Knowledge"]
    B --> D["SkillGraph"]
    D --> E["Reusable Skills"]
    C --> F["Agent Work"]
    E --> F

    G["CRM / Ads / Analytics"] --> H["Praxis Reach"]
    H --> I["Evidence Cards"]
    I --> J["Context Packs"]
    J --> F

    K["Client Capsules"] --> L["Reach for Agencies"]
    L --> H
```
...
flowchart LR
    ...
    B --> C["Create provisional graph memory"]
    C --> D["Audit / inspect / rollback"]
    D --> E["Chunk and embed"]
...
```mermaid
flowchart LR
    A["Source system"] --> B["Read-only connector"]
    B --> C["Query manifest"]
    C --> D["Evidence card"]
    D --> E["Freshness + conflict checks"]
    E --> F["Context pack"]
    F --> G["Agent work"]
```
...
```mermaid
flowchart LR
    A["Client capsule"] --> B["Systems / field maps / metrics / permissions"]
    B --> C["Reach query"]
    C --> D["Client evidence"]
    D --> E["Client context pack"]
    E --> F["Reusable agency workflow"]
```
