...
class ReachAgencyTests(unittest.TestCase):
    ...
    def test_demo_command_runs_reach_and_agency_fixture_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertIn("Stack Overflow Developer Survey AI Tooling Mini Dataset", core.stdout)
            self.assertIn("priority_breakdown:", core.stdout)
            self.assertIn("trust=", core.stdout)
            self.assertIn("freshness=", core.stdout)
            self.assertIn("priority_reasons:", core.stdout)
            self.assertIn("source_id: src:stackoverflow-dev-survey-ai-tooling-mini", core.stdout)
            self.assertIn("capture_id:", core.stdout)
            self.assertTrue(ws(root, "research", "demo_sources", "stackoverflow_developer_survey", "source_manifest.json").exists())
